...
function start() {
    ...
    if (!simulationState.chartIntervalId) {
        simulationState.chartIntervalId = setInterval(() => {
            ...
            // 移除直接的DOM写入，指标显示统一由 ui-manager.updateIndicatorsPanel 驱动
            // 此处仅保留内部能量计算以供其它模块使用（若需要）。
        }, 300);
    }
}
...
function reset(particleCount, temperature) {
    ...
    simulationState.tracerData = { collisionCount: 0, totalDistance: 0, meanFreePath: 0, mfpLengths: [] };
...
function setupTracerMetricsUpdater() {
    ...
    Events.on(engine, 'afterUpdate', () => {
        ...
        // 维护轨迹队列（滑动窗口，避免内存增长）
        const maxTrailLen = 400; // 保留最近约400帧的轨迹
        p.trail = p.trail || [];
...
function setupTracerCollisionTracker() {
    ...
    Events.on(engine, 'collisionStart', event => {
        ...
        for (const pair of pairs) {
            ...
            const mfp = td / cc;
            // 维护自由程样本数组（滑动窗口防止内存增长）
            let lengths = Array.isArray(simulationState.tracerData.mfpLengths) ? simulationState.tracerData.mfpLengths.slice() : [];
            if (Number.isFinite(d) && d > 0) {
                lengths.push(d);
                const MAX_MFP_SAMPLES = 1000;
                if (lengths.length > MAX_MFP_SAMPLES) {
                    lengths.splice(0, lengths.length - MAX_MFP_SAMPLES);
                }
            }
            ...
            // 向 UI 层广播最新的追踪器指标
            try {
              window.dispatchEvent(new CustomEvent('simulation:tracerMetrics', { detail: { tracerData: simulationState.tracerData } }));
            } catch (e) { /* ignore */ }
...
window.SimulationModule = {
    ...
    // 新增：提供追踪数据读取接口
    getTracerData: () => {
        const td = simulationState.tracerData || { collisionCount: 0, totalDistance: 0, meanFreePath: 0, mfpLengths: [] };
        return {
            collisionCount: td.collisionCount || 0,
            totalDistance: td.totalDistance || 0,
            meanFreePath: td.meanFreePath || 0,
            mfpLengths: Array.isArray(td.mfpLengths) ? td.mfpLengths.slice() : []
        };
    },
    // 新增：重置追踪统计
    resetTracerStats
