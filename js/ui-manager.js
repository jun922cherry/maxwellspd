...
export function updateTheoreticalValues() {
  ...
  console.log(`Theoretical values updated: vp=${vp.toFixed(1)}, vrms=${vrms.toFixed(1)}`);
}
...
export function initMFPViewListeners() {
  try {
    // 监听模拟模块抛出的追踪器指标事件
    window.addEventListener('simulation:tracerMetrics', (evt) => {
      const detail = evt && evt.detail ? evt.detail : {};
      const data = detail.tracerData || detail; // 支持两种负载形式
      updateMFPView(data);
    });
  } catch (e) {
    console.warn('initMFPViewListeners 失败:', e);
  }
}

// 暴露到 window 以便其它模块调用
window.updateMFPView = function(tracerData = {}) {
  try {
    // 已移除MFP图表与上下文面板，仅保留左侧指标的数值更新由主循环负责
    // 这里定义为空操作以保证事件监听兼容
  } catch (e) {
    console.warn('updateMFPView 失败:', e);
  }
};
window.initMFPViewListeners = initMFPViewListeners;
 
 // 新增：根据状态控制MFP视图显示/隐藏
 export function updateMFPViewVisibility() {
   try {
     const st = (window.State && window.State.getState && window.State.getState()) || window.appState || {};
     const visible = !!st.isMFPViewVisible;
     const mfpContainer = document.getElementById('mfp-view-container');
     const chartPanel = document.getElementById('chart-panel');
     if (mfpContainer) mfpContainer.style.display = visible ? 'flex' : 'none';
     if (chartPanel) chartPanel.style.display = visible ? 'none' : 'block';
   } catch (e) { console.warn('updateMFPViewVisibility 失败:', e); }
 }
 window.updateMFPViewVisibility = updateMFPViewVisibility;
 
 // 新增：统一指标面板更新与格式化，避免科学计数法与闪烁
 export function updateIndicatorsPanel(metrics = {}) {
   try {
     const {
       // 模拟值
       simulatedVp, simulatedVrms, entropy, simulatedTemp, simulatedN, simulatedKETotal, simulatedKEAvg, simulatedPressureRel, simulatedMFP,
       // 理论值（可选）
       theoreticalVp, theoreticalVrms, theoreticalEntropy, theoreticalTemp, theoreticalN, theoreticalKETotal, theoreticalKEAvg, theoreticalPressureRel, theoreticalTimeScale, theoreticalMFP
     } = metrics;
 
     // 工具：安全写入
     const setText = (id, valStr) => { const el = document.getElementById(id); if (el && typeof valStr === 'string') el.textContent = valStr; };
 
     // 理论值（存在时才写）
     if (Number.isFinite(theoreticalVp)) setText('vp-theoretical', theoreticalVp.toFixed(1));
     if (Number.isFinite(theoreticalVrms)) setText('vrms-theoretical', theoreticalVrms.toFixed(1));
     if (Number.isFinite(theoreticalEntropy)) setText('entropy-theoretical', theoreticalEntropy.toFixed(3));
     if (Number.isFinite(theoreticalTemp)) setText('temp-theoretical', theoreticalTemp.toFixed(2));
     if (Number.isFinite(theoreticalN)) setText('n-theoretical', String(Math.round(theoreticalN)));
     if (Number.isFinite(theoreticalKETotal)) setText('ke-total-theoretical', theoreticalKETotal.toExponential(3));
     if (Number.isFinite(theoreticalKEAvg)) setText('ke-avg-theoretical', theoreticalKEAvg.toExponential(3));
     if (Number.isFinite(theoreticalPressureRel)) setText('p-theoretical', theoreticalPressureRel.toFixed(3));
     if (Number.isFinite(theoreticalTimeScale)) setText('time-scale-theoretical', theoreticalTimeScale.toFixed(1) + 'x');
     if (Number.isFinite(theoreticalMFP)) setText('mfp-theoretical', theoreticalMFP.toExponential(3));
 
     // 模拟值（统一格式化）
      if (Number.isFinite(simulatedVp)) setText('vp-simulated', simulatedVp.toFixed(1));
      if (Number.isFinite(simulatedVrms)) setText('vrms-simulated', simulatedVrms.toFixed(1));
      if (Number.isFinite(entropy)) setText('entropy-relative', entropy.toFixed(3));
      if (Number.isFinite(simulatedTemp)) setText('macro-temp', simulatedTemp.toFixed(2));
      if (Number.isFinite(simulatedN)) setText('macro-N', String(Math.round(simulatedN)));
      if (Number.isFinite(simulatedKETotal)) setText('kinetic-energy-total', simulatedKETotal.toExponential(3));
      if (Number.isFinite(simulatedKEAvg)) setText('kinetic-energy-avg', simulatedKEAvg.toExponential(3));
      if (Number.isFinite(simulatedPressureRel)) setText('macro-P', simulatedPressureRel.toFixed(3) + 'x');
      if (Number.isFinite(simulatedMFP)) setText('mfp-real-time', simulatedMFP.toExponential(3));

      // 缓存原始数值，供导出CSV使用
      window.UIIndicatorsCache = {
        vpTheoretical: theoreticalVp,
        vrmsTheoretical: theoreticalVrms,
        entropy: entropy,
        temperatureEffective: simulatedTemp,
        particleCount: simulatedN,
        keTotal: simulatedKETotal,
        keAverage: simulatedKEAvg,
        pressureRelative: simulatedPressureRel,
        timeScale: (window.SimulationModule?.state?.timeScale ?? 1.0) + 'x',
        mfpTheoretical: theoreticalMFP,
        mfpSimulated: simulatedMFP,
        vpSimulated: simulatedVp,
        vrmsSimulated: simulatedVrms
      };
    } catch (e) {
      console.warn('updateIndicatorsPanel 失败:', e);
    }
  }
  window.updateIndicatorsPanel = updateIndicatorsPanel;
  
  // 提供当前指标数据的访问接口，供CSV导出使用
  export function getCurrentIndicatorsData() {
    try {
      const cache = window.UIIndicatorsCache || {};
      // 返回最近一次 updateIndicatorsPanel 写入的原始数值（未格式化），避免从DOM解析错误
      return { ...cache };
    } catch (e) {
      console.warn('getCurrentIndicatorsData 失败:', e);
      return null;
    }
  }
  window.getCurrentIndicatorsData = getCurrentIndicatorsData;