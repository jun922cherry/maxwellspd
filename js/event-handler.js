...
export function initTracerInteractions() {
  ...
  const handleTraceToggle = () => {
    ...
    if (mode === 'inactive') {
      ...
      if (typeof window.updateActiveChartView === 'function') {
        try { window.updateActiveChartView('mfp'); } catch (e) { /* ignore */ }
      }
    } else {
      // 退出追踪模式：恢复模拟，并清理状态
      hideTracerOverlay();
      ...
      window.State.updateState({ tracerMode: 'inactive', tracedParticleId: null, isMFPViewVisible: false });
  ...
  const handleCanvasClick = (evt) => {
    ...
    // 显示MFP视图、重置收敛历史与图表
    window.State.updateState({ tracedParticleId: clicked.id, tracerMode: 'active', isMFPViewVisible: true });
    try { window.State.resetMFPConvergenceHistory(); } catch (e) { /* ignore */ }
    try { window.MFPCharts && typeof window.MFPCharts.resetMFPCharts === 'function' && window.MFPCharts.resetMFPCharts(); } catch (e) { /* ignore */ }
    if (typeof window.updateMFPViewVisibility === 'function') { try { window.updateMFPViewVisibility(); } catch (e) { /* ignore */ } }
    // 恢复模拟并开始追踪
    hideTracerOverlay();
    setCanvasCursor('default');
...
export async function handleConclusionSubmission() {
  ...
  // 新增：根据教程索引映射为“实验步骤ID”（1..3），避免包含欢迎页导致偏移
  const rawIndex = window.currentStepIndex || 0;
  const stepIdMap = { 2: 1, 3: 2, 4: 3 };
  const stepId = stepIdMap[rawIndex] ?? 1;
  // 【关键日志】确认我们发送的 stepId 是否正确
  console.log(`Submitting conclusion for Step ID: ${stepId} (rawIndex=${rawIndex})`);
  // 无效标记：用于前端控制是否解锁下一步
  let isInvalid = false;
  try {
    const st = getState();
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (isInvalid) {
        // 当判定为无效时，避免展示任何可能的“赞扬语”。只给出提示与正确核心结论。
        addMessageToChat('assistant', '该结论无效（不相关或无法理解）。请重新上传或完善后再提交。');
        try {
          const coreLine = (critiqueText || '').split('\n').find(l => l.trim().startsWith('核心结论'));
          if (coreLine) addMessageToChat('assistant', coreLine.trim());
        } catch (_) { /* ignore */ }
        console.log('Invalid conclusion flagged by backend:', data);
      } else {
    ...
    try {
      if (!isInvalid) {
        ...
        updateState({ feedbackSubmitted: false });
        console.log('结论被判定为无效，本步不会解锁下一步。');
      }
    } catch (e) { /* ignore */ }
...
export async function handleEvaluationRequest() {
  ...
  try {
    ...
    // 4. 兼容 Python 服务返回的包装结构 { ok, data, rawText }
    let evaluationObj = resultPayload && typeof resultPayload === 'object'
      ? (resultPayload.data || resultPayload)
      : resultPayload;
    // 兜底：如果只有 rawText，尝试解析
    if (!evaluationObj || !evaluationObj.dimensions) {
      try {
        const raw = (resultPayload && resultPayload.rawText) ? resultPayload.rawText : '';
        if (raw && typeof raw === 'string') {
          const parsed = JSON.parse(raw);
          if (parsed && typeof parsed === 'object') evaluationObj = parsed;
        }
      } catch (_) { /* ignore */ }
    }

    // 5. 存储结果并更新 UI
    updateState({ evaluationResult: evaluationObj, isLoadingEvaluation: false });
    hideLoadingModal();
    showEvaluationReportModal(evaluationObj);
  } catch (error) {
