...
export const appState = window.appState || {
  ...
  // ===== 新版MFP视图状态 =====
  isMFPViewVisible: false,
  mfpInfluenceVariable: 'N', // 'N' 或 'T'
  mfpDistributionData: { bins: [], counts: [] },
  mfpConvergenceHistory: [],
  // ===== 新增：智能评价系统状态 =====
  isFinalized: false,
...
window.State = {
  ...
  appendMFPConvergencePoint: (pt) => {
    try {
      if (!Array.isArray(appState.mfpConvergenceHistory)) appState.mfpConvergenceHistory = [];
      appState.mfpConvergenceHistory.push(pt);
      const limit = window.MAX_MFP_HISTORY || 500;
      while (appState.mfpConvergenceHistory.length > limit) appState.mfpConvergenceHistory.shift();
      window.appState = appState;
      return appState.mfpConvergenceHistory.length;
    } catch (e) { console.warn('appendMFPConvergencePoint 失败:', e); return appState.mfpConvergenceHistory.length; }
  },
  resetMFPConvergenceHistory: () => { appState.mfpConvergenceHistory = []; window.appState = appState; }
};

// ===== 新增：直方图时间平滑窗口大小（常量） =====
export const HISTOGRAM_SMOOTHING_FRAMES = 15; // 平滑窗口大小（帧数）
// ===== 新增：MFP收敛历史最大长度（常量） =====
export const MAX_MFP_HISTORY = 500;
window.HISTOGRAM_SMOOTHING_FRAMES = HISTOGRAM_SMOOTHING_FRAMES;
window.MAX_MFP_HISTORY = MAX_MFP_HISTORY;