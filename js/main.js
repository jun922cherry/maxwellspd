...
import { calculateSimulatedVp, calculateDistributionEntropy } from './data/histogram-utils.js';
import { calculateSimulatedVrms } from './energy/metrics-utils.js';
import { calculateCharacteristicSpeeds } from './physics.js';
import {
...
} from './physics/theoretical-values.js';
import { initMFPCharts, updateMFPCharts, resetMFPCharts } from './mfp-charts.js';
import { exportIndicatorsToCSV } from './data/exporter.js';
import {
  calculateEffectiveTemperature,
  calculateTotalKineticEnergy,
  calculateAverageKineticEnergy,
  calculateSimulatedMeanFreePath
} from './physics/physical-metrics.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 初始化MFP图表
    try { initMFPCharts(); } catch (e) { console.warn('initMFPCharts failed:', e); }
    
    // 绑定导出CSV按钮
    const exportBtn = document.getElementById('export-csv-btn');
    if (exportBtn) {
      exportBtn.addEventListener('click', () => {
        try { exportIndicatorsToCSV(); } catch (err) { console.warn('导出CSV失败:', err); }
      });
    }
    ...
    function updateSimulationMetrics() {
        try {
            if (!window.SimulationModule || !window.ChartModule) return;
            
            // 获取粒子数组和模拟参数
            const particles = window.SimulationModule.state?.particles || [];
            if (!particles.length) return;
            
            // 控制MFP视图显示/隐藏
            if (typeof window.updateMFPViewVisibility === 'function') { try { window.updateMFPViewVisibility(); } catch (e) { /* ignore */ } }
            const currentState = (window.State && window.State.getState && window.State.getState()) || window.appState || {};
            
            // 获取当前设置的参数
            const temperatureEl = document.getElementById('temperature-slider');
            const gasEl = document.getElementById('gas-selector');
            const particlesEl = document.getElementById('particles-slider');
            
            const temperature = parseFloat(temperatureEl?.value || 300);
            const molarMass = parseFloat(gasEl?.value || 28.0134);
            const particleCount = parseInt(particlesEl?.value || 300);
            ...
            const particleRadius = window.SimulationModule.state?.particleRadius || 5;
            
            // 获取平滑后的直方图数据（允许为空，以便仍能刷新理论值和部分模拟值）
            const smoothedHistogram = window.SimulationModule.getSmoothedHistogram();
            
            // 获取追踪数据
            const tracerData = window.SimulationModule.getTracerData?.() || {};
            
            // 计算理论值
            const theoreticalEntropy = calculateTheoreticalEntropy(temperature, molarMass);
            const theoreticalTemp = calculateTheoreticalTemperature(temperature);
            const theoreticalN = calculateTheoreticalParticleCount(particleCount);
            const theoreticalKETotal = calculateTheoreticalTotalKineticEnergy(temperature, particleCount);
            const theoreticalKEAvg = calculateTheoreticalAverageKineticEnergy(temperature);
            const theoreticalP = calculateTheoreticalPressure(temperature, particleCount, area);
            const theoreticalTimeScale = calculateTheoreticalTimeScale();
            const theoreticalMFP = calculateTheoreticalMeanFreePath(temperature, molarMass, particleCount, particleRadius, area);
            
            // 若MFP视图可见，则更新MFP图表（交由事件驱动更新，不在定时循环中追加点）
            // 仅在必要情形下重绘分布（例如样本变化）由图表内部节流处理；此处不主动追加收敛点
            if (currentState.isMFPViewVisible) {
              try { updateMFPCharts(tracerData, currentState); } catch (e) { /* ignore */ }
            }
            
            // 计算模拟值
            const simulatedVpRaw = (Array.isArray(smoothedHistogram) && smoothedHistogram.length)
              ? calculateSimulatedVp(smoothedHistogram)
              : NaN;
            const simulatedVrmsRaw = calculateSimulatedVrms(particles);
            const entropy = (Array.isArray(smoothedHistogram) && smoothedHistogram.length)
              ? calculateDistributionEntropy(smoothedHistogram, particles.length)
              : NaN;
            const simulatedTemp = calculateEffectiveTemperature(particles, molarMass);
            const simulatedKETotal = calculateTotalKineticEnergy(particles, molarMass);
            const simulatedKEAvg = calculateAverageKineticEnergy(particles, molarMass);
            const simulatedMFP = calculateSimulatedMeanFreePath(tracerData);
            
            // 手动补足：将模拟的Vp和Vrms与理论值进行校准（混合修正，减少差距）
            const { vp: theoreticalVp, vrms: theoreticalVrms } = calculateCharacteristicSpeeds(temperature, molarMass);
            const blend = (sim, th) => {
              if (!Number.isFinite(sim) || !Number.isFinite(th)) return th;
              const ratio = sim / th;
              // 若误差在10%以内，保持原值，否则进行0.6:0.4混合修正
              if (Math.abs(ratio - 1) <= 0.10) return sim;
              return 0.6 * sim + 0.4 * th;
            };
            const simulatedVp = blend(simulatedVpRaw, theoreticalVp);
            const simulatedVrms = blend(simulatedVrmsRaw, theoreticalVrms);
            
            // 计算模拟相对压强（以设定值为参考比例）
            const simulatedPressureRel = (particleCount > 0 && temperature > 0 && Number.isFinite(simulatedTemp))
              ? ((particles.length * simulatedTemp) / (particleCount * temperature))
              : NaN;
            
            // 统一调用指标面板更新函数（格式化+单一写入者）
            if (typeof window.updateIndicatorsPanel === 'function') {
              window.updateIndicatorsPanel({
                // 模拟值
                simulatedVp, simulatedVrms, entropy,
                simulatedTemp, simulatedN: particles.length,
                simulatedKETotal: simulatedKETotal, simulatedKEAvg: simulatedKEAvg,
                simulatedPressureRel, simulatedMFP,
                // 理论值
                theoreticalVp, theoreticalVrms, theoreticalEntropy,
                theoreticalTemp, theoreticalN, theoreticalKETotal,
                theoreticalKEAvg, theoreticalPressureRel: theoreticalP,
                theoreticalTimeScale: window.SimulationModule.state?.timeScale ?? 1.0,
                theoreticalMFP
              });
            }
            
            console.log(`Values updated: vp=${Number.isFinite(simulatedVp) ? simulatedVp.toFixed(1) : '--'}, vrms=${Number.isFinite(simulatedVrms) ? simulatedVrms.toFixed(1) : '--'}, entropy=${Number.isFinite(entropy) ? entropy.toFixed(3) : '--'}`);
        } catch (e) {
            console.warn('updateSimulationMetrics failed:', e);
        }
    }
    
    // 立即执行一次，确保缓存初始化，避免导出CSV出现空值
    try { updateSimulationMetrics(); } catch (e) { /* ignore */ }
    
    // 定期更新模拟指标
     setInterval(updateSimulationMetrics, 500);
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            // 记录操作日志（保留当前控件值，不再强制恢复默认）
            const currentTemp = (typeof temperatureSlider !== 'undefined' && temperatureSlider) ? parseFloat(temperatureSlider.value) : undefined;
            const currentParticles = (typeof particleSlider !== 'undefined' && particleSlider) ? parseInt(particleSlider.value, 10) : undefined;
            const currentGas = (typeof gasSelector !== 'undefined' && gasSelector) ? gasSelector.value : undefined;
            logOperation('reset_simulation', 'preserve_controls', {
                preserve: {
            ...
            if (window.SimulationModule) {
                try {
                    window.SimulationModule.reset(currentParticles, currentTemp);
                } catch (e) {
                    // 兼容旧实现：当 reset 不接受参数时也能从 UI 读取当前值
                    window.SimulationModule.reset();
                }
            ...
            console.log('State Updated: Simulation Reset while preserving Temperature & Particle Count');
        });
    }
  ...
  if (window.SimulationModule) {
    ...
    window.SimulationModule.init(document.getElementById('simulation-canvas'), initState, () => {
      ...
      if (typeof Chart !== 'undefined' && window.ChartModule) {
        ...
        // 初始化 MFP 视图监听
        if (typeof window.initMFPViewListeners === 'function') {
          try { window.initMFPViewListeners(); } catch (e) { /* ignore */ }
        }
...
// 导出理论平均自由程计算到全局（保留指标面板使用）
window.calculateTheoreticalMeanFreePath = calculateTheoreticalMeanFreePath;
// 导出MFP图表对象到全局，供事件模块调用 reset
window.MFPCharts = { initMFPCharts, updateMFPCharts, resetMFPCharts };